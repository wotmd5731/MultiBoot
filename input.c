...
int O_JAE_touch(int *x, int *y){
      ...
      {	
            ...
            while(ev_get(&ev, 1) == 0)
            {
                  //ERROR("O_JAE ev_get %d %d %d \n",ev.type,ev.code,ev.value);
                  switch(ev.type)
                  {
                        case EV_KEY:
                              //handle_key_event(&ev);
                              if(ev.value==1){
                                    //ERROR("O_JAE key press\n");
                              }else{
                                    //ERROR("O_JAE key released input_run =0 \n");
                                    return 1;
                                    //input_run=0;         
                              }      
                              break;
                        case EV_ABS:
                              if(ev.code==53){
                                    if(input_run==1){
                                          //ERROR("O_JAE Xpos %d\n",ev.value);
                                          *x = ev.value*fb_get_vi_xres()/(mt_range_x[1]-mt_range_x[0]);
                                    }
                              }else if(ev.code==54){
                                    if(input_run==1){
                                          //ERROR("O_JAE Ypos %d\n",ev.value);
                                          *y = ev.value*fb_get_vi_yres()/(mt_range_y[1]-mt_range_y[0]);
                                    }
                              }else if(ev.code==57){
                                    if(ev.value>=0){
                                    
                                          //ERROR("O_JAE key abs press\n");
                                          input_run=1;
                                    }else{
                                   
                                          //ERROR("O_JAE key abs released input_run =0 \n");
                                          return 1;
                                          //input_run=0;         
                                    }   
                              }   
