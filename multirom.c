...
ERROR("O_JAE func mutlirom  char *rom_to_boot %s \n",rom_to_boot);

//multirom_copy_log(NULL, "../multirom_second_log.txt");
...
ERROR("O_JAE  multirom_ui passed   rom_to_boot = %s \n",rom_to_boot);
ERROR("      rom name %s \n",to_boot->name);
...
//umount("/system");
    ...
    if(to_boot)
    {
		...
		ERROR("O_JAE end of prepare_for_boot :%d \n",exit);
...
//printf("O_JAE to_boot->name : %s \n",to_boot->name);
finish: 
    read_directory("/");
ERROR("O_JAE  boot finish call -> main return exit value = %d \n ",exit);
sprintf(bufff,"../log_%s.txt",to_boot->name);
multirom_copy_log(NULL, bufff);
...
ERROR("O_JAE type : %d \n",type);
ERROR("O_JAE to_boot->has_bootimg : %d \n",to_boot->has_bootimg);
ERROR("O_JAE type != ROM_DEFAULT : %d \n",(type != ROM_DEFAULT));
ERROR("O_JAE s->is_second_boot == 0 : %d \n",(s->is_second_boot == 0));


/*if(s->is_second_boot == 0){
ERROR("force is_seconde_boot = 1 ");
s->is_second_boot =1;
}*/
    if(((M(type) & MASK_KEXEC) || to_boot->has_bootimg) && type != ROM_DEFAULT && s->is_second_boot == 0)
    {
    	ERROR("O_JAE into call Mask all");
    ...
    switch(type)
    {
        ...
        case ROM_ANDROID_INTERNAL:
        {
            ...
            {	
            	ERROR("O_JAE android internal into IF \n");
                exit &= ~(EXIT_UMOUNT);
...
chmod(out,0777);
    ...
    mkdir_with_perms("/system", 0777, NULL, NULL);
    mkdir_with_perms("/data", 0777, "system", "system");
    mkdir_with_perms("/cache", 0777, "system", "cache");
    if(has_fw)
        mkdir_with_perms("/firmware", 0777, "system", "system");
    ...
    for(i = 0; i < ARRAY_SIZE(folders[0]); ++i)
    {
		...
		ERROR("O_JAE forloop from: %s to: %s \n",from,to);
        if(img == 0)
        {
        ERROR("O_JAE img==0");
