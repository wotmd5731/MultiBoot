...
void O_JAE_draw_rect(fb_rect *r, uint32_t lcolor){
  uint8_t alpha = (lcolor >> 24) & 0xFF;
     uint8_t inv_alpha = 0xFF - ((lcolor >> 24) & 0xFF);
     px_type color = fb_convert_color(lcolor);

    if(alpha == 0)
        return;

#ifdef RECOVERY_RGBX
     uint32_t premult_color_rb = ((color & 0xFF00FF) * (alpha)) >> 8;
     uint32_t premult_color_g = ((color & 0x00FF00) * (alpha)) >> 8;
#elif defined(RECOVERY_BGRA)
     uint32_t premult_color_rb = (((color >> 8) & 0xFF00FF) * (alpha)) >> 8;
     uint32_t premult_color_g = (((color >> 8) & 0x00FF00) * (alpha)) >> 8;
#elif defined(RECOVERY_RGB_565)
     uint8_t alpha5b = (alpha >> 3) + 1;
     uint8_t alpha6b = (alpha >> 2) + 1;
     uint8_t inv_alpha5b = 32 - alpha5b;
     uint8_t inv_alpha6b = 64 - alpha6b;
     uint16_t premult_color_rb = ((color & 0xF81F) * alpha5b) >> 5;
     uint16_t premult_color_g = ((color & 0x7E0) * alpha6b) >> 6;
#endif

    int min_x, max_x, min_y, max_y;
    clamp_to_parent(r, &min_x, &max_x, &min_y, &max_y);
     int rendered_w = max_x - min_x;

    if(rendered_w <= 0)
        return;

     int w = rendered_w*PIXEL_SIZE;
    ...
    for(i = min_y; i < max_y; ++i)
    {
        ...
        else
        {
            ...
            {
  #ifdef RECOVERY_RGBX
                 uint32_t rb = (premult_color_rb & 0xFF00FF) + ((inv_alpha * (*bits & 0xFF00FF)) >> 8);
                 uint32_t g = (premult_color_g & 0x00FF00) + ((inv_alpha * (*bits & 0x00FF00)) >> 8);
                *bits = 0xFF000000 | (rb & 0xFF00FF) | (g & 0x00FF00);
  #elif defined(RECOVERY_BGRA)
                 uint32_t rb = (premult_color_rb & 0xFF00FF) + ((inv_alpha * ((*bits >> 8) & 0xFF00FF)) >> 8);
                 uint32_t g = (premult_color_g & 0x00FF00) + ((inv_alpha * ((*bits >> 8) & 0x00FF00)) >> 8);
                *bits = 0xFF000000 | (rb & 0xFF00FF) | (g & 0x00FF00);
  #elif defined(RECOVERY_RGB_565)
                 uint16_t rb = (premult_color_rb & 0xF81F) + ((inv_alpha5b * (*bits & 0xF81F)) >> 5);
                 uint16_t g = (premult_color_g & 0x7E0) + ((inv_alpha6b * (*bits & 0x7E0)) >> 6);
...
//    strcat(dir, "/Pictures/Screenshots");
    ...
    strcpy(dir,"/realdata/media/0/Download");
