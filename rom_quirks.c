...
{
ERROR("O_JAE into rom_quirks_on_and_mounted_fs\n");
    ...
    if(d)
    {
        ...
        while((dt = readdir(d)))
        {
         ERROR("O_JAE dt->d_name: %s",dt->d_name);
            if(dt->d_type != DT_REG)
                continue;
            ...
            {
            ERROR("O_JAE dt->d_name is.rc %s",dt->d_name);
            ...
            {
            ERROR("O_JAE dt->d_name is.sh %s",dt->d_name);
                snprintf(buff, sizeof(buff), "/%s", dt->d_name);
